...
class Resolver:
    ...
    def resolve(self, query: str, context: ShellContext, op: str = "generate") -> ResolveResult:
        ...
        ctx_sensitive = is_context_sensitive(query)
        ...
        try:
            query_vec: np.ndarray = self.embedder.embed(query, self.embedding_model)
        except Exception as exc:
            log.warning("Embedding failed, falling through to Tier 3: %s", exc)
            query_vec = None

        best_history_cmd: str | None = None  # Best Tier-2 result for Tier-3 context

        if query_vec is not None:
            # --- Tier 1: semantic cache ---
            result, best_cache_score = self._tier1(query_vec, ctx_hash, ctx_sensitive)
            if result:
    ...
    def _tier1(
        ...
        rows = self.store.semantic_get_all(ctx_hash if ctx_sensitive else None)
        if not rows:
            return None, 0.0
        ...
        row = rows[best_idx]
        # Context guard: if context-sensitive and context_hash set, ensure match
    ...
    def _tier2(
        ...
        rows = self.store.history_get_all()
        if not rows:
            return None, None
        ...
        # Below threshold: still return as context hint for Tier 3
    ...
    def _tier3(
        ...
        # If streaming, collect to string
        if hasattr(gen_result, "__iter__") and not isinstance(gen_result, GenResult):
            gen_result = self._collect_stream(gen_result, self.primary_provider_name, self.primary_model)

        if gen_result.error:
            log.warning("Primary provider error: %s", gen_result.error)
            return self._escalate(query, norm, context, dynamic_user, query_vec, gen_result.error)
        ...
        if not validation.is_valid:
            ...
            return self._escalate(query, norm, context, dynamic_user, query_vec, validation.parse_error)

        # Success — write to caches
    ...
    def _escalate(
        ...
        # Feed the validation error back in so the stronger model can avoid the issue
        escalate_user = dynamic_user
        if prev_error:
            escalate_user = f"{dynamic_user}\n\n[Previous attempt failed: {prev_error}. Generate a corrected command only.]"
        ...
        if hasattr(gen_result, "__iter__") and not isinstance(gen_result, GenResult):
            gen_result = self._collect_stream(gen_result, self.escalate_provider_name, self.escalate_model)
        ...
        # Write to cache even if parse warn (command still goes to buffer)
        ctx_hash = context.context_bucket_hash() if is_context_sensitive(query) else None
        self._write_caches(norm, query, query_vec, gen_result.text, ctx_hash,
                           gen_result.provider, gen_result.model)
    ...
    def _build_user_message(
        ...
        if history_hint:
            hint = redact_secrets(history_hint) if self.redact else history_hint
            parts.append(f"Relevant history: {hint}")

        if op == "explain":
            parts.append(f"Explain this command concisely: {context.buffer}")
        elif op == "fix_last":
            parts.append(
                f"The last command failed (exit {context.last_exit_code}): {context.buffer}\n"
                f"Intent: {query}"
            )
        else:
    ...
    @staticmethod
    def _collect_stream(stream, provider_name: str, model: str) -> GenResult:
        chunks: list[str] = []
        try:
            for chunk in stream:
                chunks.append(chunk)
        except Exception as exc:
            return GenResult(text="", tokens_in=0, tokens_out=0, cached_tokens=0,
                             model=model, provider=provider_name, error=str(exc))
        text = "".join(chunks).strip()
        return GenResult(text=text, tokens_in=0, tokens_out=0, cached_tokens=0,
